...
st.set_page_config(
    page_title="Visor de Inventario Pro - Enhanced",
    page_icon="📦",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Componente personalizado para navegación con teclado
def keyboard_navigation_component():
    """Componente personalizado para mejorar la navegación con teclado"""
    keyboard_js = """
    <script>
    ...
    function initializeKeyboardNavigation() {
        ...
        fields = Array.from(document.querySelectorAll('input[type="text"], input[type="number"]'))
            .filter(input => input.offsetParent !== null); // Solo campos visibles
    ...
    window.reinitKeyboard = initializeKeyboardNavigation;
    </script>
...
def main():
    ...
    with st.sidebar:
        ...
        st.session_state.auto_focus_enabled = st.checkbox(
            "Auto-focus después de agregar", 
            value=st.session_state.auto_focus_enabled,
            help="Automáticamente enfocar el primer campo después de agregar un pallet"
        )
    ...
    if not st.session_state.archivo_cargado:
        ...
        with st.container():
            st.markdown('<div class="input-container active">', unsafe_allow_html=True)
        ...
        st.info("🚀 **Navegación rápida**: Tablilla → Enter → ID Pallet → Enter → Cantidad → Enter (agregar automáticamente)")
        ...
        if st.session_state.conteo_fisico:
            ...
            if not df_filtered.empty:
                ...
                with col2:
                    # Generar reporte Excel (función existente mejorada)
                ...
                with col3:
                    if st.button("📈 Dashboard Avanzado", use_container_width=True):
                        st.session_state.show_advanced_dashboard = True
                        st.rerun()
...
<div style='text-align: center; color: #666; padding: 2rem;'>
    ...
    <p><em>Versión mejorada con componentes personalizados y dashboard empresarial</em></p>
</div>
""", unsafe_allow_html=True)